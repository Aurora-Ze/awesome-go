...
func TestDay5() {
	engine := day52.Default()
	engine.GET("/", func(context *day52.Context) {
		time.Sleep(time.Millisecond * 10)
		context.JSON(http.StatusOK, "hello")
	})
	engine.POST("/recovery", func(context *day52.Context) {
		arr := []int{1, 2}
		fmt.Println(arr[4])

		context.JSON(http.StatusOK, day52.H{
			"msg": "haha",
		})
	})
	engine.POST("/post", func(context *day52.Context) {
